 ...
 * <p>
 * Provides a fast probabilistic check before hitting the database.
 * If the Bloom filter says "not present", the email is definitely new.
 * If it says "possibly present", a database query confirms.
 */
public interface RegistrationValidationService {
     ...
     * @return {@code true} if the email is already registered (confirmed by DB),
     *         {@code false} if it is available
     */
